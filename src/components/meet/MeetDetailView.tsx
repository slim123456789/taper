    ...
    <main className="flex min-h-screen justify-center bg-gradient-to-b from-[#02030A] to-[#020107] px-4 py-8">
      <div className="w-full max-w-[430px] rounded-3xl border border-white/10 bg-[#070A14] px-5 pb-9 pt-7 shadow-[0_26px_70px_rgba(0,0,0,0.85)] text-slate-50">
        ...
        <div className="mb-4 flex rounded-lg bg-white/5 p-1">
          <button onClick={() => setViewMode("upcoming")} className={`flex-1 rounded-md py-1.5 text-[11px] font-medium uppercase tracking-wider transition ${viewMode === "upcoming" ? "bg-slate-700 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"}`}>Upcoming</button>
          <button onClick={() => setViewMode("results")} className={`flex-1 rounded-md py-1.5 text-[11px] font-medium uppercase tracking-wider transition ${viewMode === "results" ? "bg-slate-700 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"}`}>Results</button>
        </div>
        ...
        {viewMode === "upcoming" && (
          <section className="mb-4 rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-3 text-[12px] text-slate-200">
            <div className="flex items-center justify-between">
              <p>Picks locked: <span className="font-semibold text-slate-50">{submittedCount}/{totalUpcoming}</span></p>
              {isLockedByTime && <span className="rounded-full border border-slate-400/50 bg-slate-500/10 px-2 py-[2px] text-[10px] font-medium text-slate-200">Locked</span>}
            </div>
            <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-slate-900/80">
              <div className="h-full bg-slate-100" style={{ width: totalUpcoming > 0 ? `${(submittedCount / totalUpcoming) * 100}%` : "0%" }} />
            </div>
          </section>
        )}
        ...
        <section className="space-y-3">
          ...
          {currentMarkets.length === 0 && (
            <div className="py-8 text-center text-[12px] text-slate-400">
              <p>{viewMode === "upcoming" ? "No active markets available." : "No final results yet."}</p>
            </div>
          )}
        </section>
      </div>
...
function MarketCard({ market, pick, submitted, onPick, onSubmit, onUnlock, lockedByTime }: MarketCardProps) {
  ...
  const handleMainAction = () => {
    ...
    if (submitted) onUnlock(market.id);
    else if (pick) onSubmit(market.id);
  };
